...
# Kernel parameters
if grep -E "zswap.enabled=1|zswap.max_pool_percent=6|zswap.compressor=zstd|transparent_hugepage=always|amd-pstate=active|skew_tick=1|amd_prefcore=enable" /etc/kernel/cmdline; then
	...
	if [[ -f /etc/kernel/cmdline ]]; then
		...
		echo -n " zswap.enabled=1 zswap.max_pool_percent=6 zswap.compressor=zstd transparent_hugepage=always amd-pstate=active skew_tick=1 amd_prefcore=enable" >>/etc/kernel/cmdline
